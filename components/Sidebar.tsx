  ...
  return (
    <aside className="fixed top-0 left-0 w-64 h-full bg-black">
      <div className="flex flex-col items-center h-full m-5 mt-5">
        <Image src="/images/spotify_logo.png" width={125} height={50} objectFit="contain" alt="Spotify logo" />

        <ul className="w-full mt-4">
          <Link href="/">
            <a>
              <li className={`${router.pathname === '/' ? activeLink : inactiveLink} flex text-sm items-center gap-3 p-2 rounded`}>
                {router.pathname === '/' ? <RiHome5Fill className="text-2xl" /> : <RiHome5Line className="text-2xl" />}
          ...
          <Link href="/collection/playlists">
            <a>
              <li
                className={`${
                  router.pathname.includes('/collection') && !router.pathname.includes('tracks') ? activeLink : inactiveLink
                } flex items-center gap-3 p-2 text-sm rounded cursor-pointer  hover:text-white`}
              >
          ...
          <Link href="/collection/tracks">
            <a>
              ...
              >
                <Image src="/images/liked_cover.jpeg" height={28} width={28} objectFit="contain" alt="Liked playlist cover" />
        ...
        <ul id="sidebar-playlists" className="flex flex-col w-full gap-3 pr-3 mt-5 overflow-x-hidden text-sm text-gray">
          {playlists?.map((playlist) => (
            <Link key={playlist.id} href={`/playlist/${playlist.id}`}>
              <a className="w-full">
                <li key={playlist.id} className="text-sm font-semibold truncate cursor-default hover:text-white">
                  {playlist.name}
                </li>
              </a>
            </Link>
