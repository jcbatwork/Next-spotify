...
export default function TracksTable({ tracks, noAlbum = false, noArtist = false }: IProps) {
  ...
  return (
    <div className="grid grid-cols-12 gap-2 p-1 mt-8">
      {!noArtist && (
        <>
          <div className="col-span-1 font-semibold tracking-wider text-left text-white uppercase">#</div>

          <div className={`${noAlbum ? 'col-span-10' : 'col-span-6'} text-sm font-medium tracking-wider text-left uppercase text-gray`}>Title</div>

          {!noAlbum && <div className="col-span-4 text-sm font-medium tracking-wider text-left uppercase text-gray">Album</div>}
      ...
      <div className="w-full col-span-12">
        {tracks?.map((track, index) => (
          <div className={`grid grid-cols-12 ${!track.preview_url ? 'opacity-50' : ''}`} key={track.id + index + 1}>
            <div className="flex items-center col-span-1 my-3 text-sm text-gray">{index + 1}</div>

            <div className={`${noAlbum ? 'col-span-10' : 'col-span-6'} flex items-center w-full  my-3`}>
