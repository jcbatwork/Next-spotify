        ...
        {album && (
          <>
            {album.images.length > 0 ? (
              <img src={album.images[0].url} alt={album.name} className="object-contain w-52 h-52" />
            ) : (
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex items-center gap-5 text-sm">
                ...
                {album.tracks.items.length > 0 && <span className="text-gray">{album.tracks.total} songs</span>}{' '}
              </div>
            </div>
          </>
        )}
...
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  ...
  const album = await customGet(`https://api.spotify.com/v1/albums/${albumId}`, session);
  return { props: { album } };
};
